...
class SingletonToken {
    ...
    async getGeneralAccessToken() {
        if(!SingletonToken.accessToken || !this.isTokenValid(SingletonToken.accessTokenLastUpdated)) {
            this.getWxAccessToken().then(response => {
                if (response.access_token) {
                    SingletonToken.accessToken = response.access_token;
                    SingletonToken.accessTokenLastUpdated = Date.now();
                    console.log(`--getGeneralToken success, response:${JSON.stringify(response)}`);
                } else {
                    console.error(`--getGeneralToken failed, response:${JSON.stringify(response)}`);
    ...
    async getJsapiTicket() {
        ...
        if (accessToken) {
            if (!SingletonToken.jsapiTicket || !this.isTokenValid(SingletonToken.jsapiTicketLastUpdated)) {
