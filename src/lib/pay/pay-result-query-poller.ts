...
export class Poller {
    ...
    private getPollInterval(): number {
        return Date.now() - this.startTime < this.earlyPeriod ? this.pollIntervalEarly : this.pollIntervalLate;
    }
    ...
    private isFinalOrderStatus(value: any): boolean {
        ...
        return Object.values(PayOrderFinalStatus).includes(value as PayOrderFinalStatus);
    }
    ...
    private async poll() {
        try {
            ...
            // 如果订单不是终态且轮训时间没到
            if (!this.isFinalOrderStatus(result?.orderStatus) && this.shouldContinue()) {
