...
const APP_ID = "2024020100006551";
const VENDOR_SN = "91802047"; 
const VENDOR_KEY = "9593be2bc9ae93516a88bfecaf018ba2"
const SHOUQIANBA_APP_DOMAIN = "https://vsi-api.shouqianba.com";
const SHOUQIANBA_PAY_DOMAIN = "https://qr.shouqianba.com/gateway";
 ...
 * @returns 
 */
export const generatePayUrl = (payRequest: PayRequest, terminal: Terminal): string => {
    const params = {
    ...
    // MD5 加密并转大写
    const sign =  createHash('md5').update(stringToSign, 'utf-8').digest('hex').toUpperCase();
    const wholeParams = sortedParamStr + "&sign=" + sign;
    const urlWithParams = SHOUQIANBA_PAY_DOMAIN + '?' + wholeParams;
 ...
 * @returns 
 */
export const activate = async (deviceId: string, code: string)
: Promise<any> => {
    const url = `${SHOUQIANBA_APP_DOMAIN}/terminal/activate`;
    const params = {
        app_id: APP_ID, 
        code: code, 
        device_id: deviceId,
    ...
    try {
        const sign = getSign(JSON.stringify(params) + VENDOR_KEY);
        console.error(`param: ${JSON.stringify(params)}`);
        console.error(VENDOR_SN + " " + sign);
        ...
        if (responseData.result_code !== '200') {
            console.error(`Activation failed with result_code, error msg: ${JSON.stringify(responseData.error_message)}`);
            throw new Error(`Activation failed with result_code: ${responseData.result_code}`);
        }
        console.log(`terminal data: ${JSON.stringify(responseData)}`);

        const terminalResult = responseData.biz_response;
        if (!terminalResult.terminal_sn || !terminalResult.terminal_key) {
            console.error(`Response missing terminal_sn or terminal_key, terminal: ${JSON.stringify(terminalResult)}`);
            throw new Error('Response missing terminal_sn or terminal_key');
        }
        const  terminal = new Terminal(terminalResult.terminal_sn, terminalResult.terminal_key);
 ...
 * @returns 
 */
export const checkin = async (terminal: Terminal, deviceId: string): Promise<any> => {
    const url = `${SHOUQIANBA_APP_DOMAIN}/terminal/checkin`;
    const params = {
        terminal_sn: terminal.terminalSn,
        device_id: deviceId,
    };
    ...
    try {
        ...
        if (responseData.result_code !== '200') {
            console.log(`Checkin failed, error msg: ${JSON.stringify(responseData.error_message)}`);
            throw new Error(`Checkin failed with result_code: ${responseData.result_code}`);
        }

        const newTerminalResult = responseData.biz_response;
        if (!newTerminalResult.terminal_sn || !newTerminalResult.terminal_key) {
            console.log(`Response missing terminal_sn or terminal_key, terminal: ${JSON.stringify(terminal)}`);
            throw new Error('Response missing terminal_sn or terminal_key');
        }
        console.log(`new terminal data: ${JSON.stringify(newTerminalResult)}`);

        const newTerminal = new Terminal(newTerminalResult.terminal_sn, newTerminalResult.terminal_key);
 ...
 * @returns 
 */
export const query = async (terminal: Terminal, orderCode: string)
: Promise<any> => {
    ...
    const url = `${SHOUQIANBA_APP_DOMAIN}/upay/v2/query`;
    const params = {
        terminal_sn: terminal.terminalSn,
        client_sn: orderCode
    };
    try{
        ...
        if (responseData.result_code !== '200') {
            console.log(`Query failed, error msg: ${JSON.stringify(responseData.error_message)}`);
            throw new Error(`Query failed with result_code: ${responseData.result_code}`);
        }
...
const cancel = async(terminal: Terminal, orderCode: string): Promise<any> => {
    ...
    const url = `${SHOUQIANBA_APP_DOMAIN}/upay/v2/cancel`;
    const params = {
        terminal_sn: terminal.terminalSn,
        client_sn: orderCode
    };
    try{
        ...
        if (responseData.result_code !== '200') {
            throw new Error(`Cancel failed with result_code: ${responseData.result_code}`);
        }
 ...
 * @returns 
 */
export const refund = async(terminal: Terminal, originalOrderCode: string,refundOrderCode: string, 
    refundAmount: string, operator: string, ): Promise<any> => {
        const url = `${SHOUQIANBA_APP_DOMAIN}/upay/v2/refund`;
        const params = {
            terminal_sn: terminal.terminalSn,
        ...
        try{
            ...
            if (responseData.result_code !== '200') {
                console.log(`Refund failed, error msg: ${JSON.stringify(responseData.error_message)}`);
                throw new Error(`Refund failed with result_code: ${responseData.result_code}`);
            }
            console.log(`responseData: ${JSON.stringify(responseData)}`);
            return new Result(true, responseData);
        } catch (error) {
